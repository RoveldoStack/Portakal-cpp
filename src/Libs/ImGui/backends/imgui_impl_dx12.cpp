    ...
    for (int n = 0; n < draw_data->CmdListsCount; n++)
    {
        ...
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
                ...
                /*
                * Check if imgui wants to push the font texture instead of custom texture
                * If wants to push the font texture it means that we do not need the custom descriptor heap
                * If it wants to push the custom texture then we will have to bind the Descriptor heap specific to that texture
                */
                if ((UINT64)pcmd->TextureId != bd->pd3dSrvDescHeap->GetGPUDescriptorHandleForHeapStart().ptr)
                {
                    ID3D12DescriptorHeap* pHeap = (ID3D12DescriptorHeap*)pcmd->GetTexID();
                    ctx->SetDescriptorHeaps(1, &pHeap);
                    ctx->SetGraphicsRootDescriptorTable(1, pHeap->GetGPUDescriptorHandleForHeapStart());
                }
                else
                {
                    ...
                    ctx->SetDescriptorHeaps(1, &bd->pd3dSrvDescHeap);
